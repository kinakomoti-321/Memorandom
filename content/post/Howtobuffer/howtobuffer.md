...
Bufferさえ使えれば、後はほとんどCPUパストレーサーと同様の処理をGLSLに翻訳してあげればこのように割かし簡単に作ることができます。
ただし私が実装した例では途中のカメラ移動等を前提としていないため、処理中に数値を変えるなどをすると最初に戻る必要があります。これをどうにかするにはカメラの位置などが変わったことを確認できるような仕組みを作って、BufferAやBufferBがリセットされるようにしなくてはいけません。（どうにかしたい）
